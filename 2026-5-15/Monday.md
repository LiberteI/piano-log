# 🎹 Piano Progress Log

## How to use
After every session, copy the template below and fill it in.

---

## Rules of Thumb
1. Practice with full attention; do not play on autopilot.
2. Prioritize control and musicality over raw BPM.
3. Practice slowly enough to stay accurate and relaxed.
4. Be honest about weak spots and work on the highest-leverage problem first.
5. Consistency matters more than intensity.
6. Stop or reset when focus is gone.

## Session Template
1. Each session belongs to a week folder.
2. Weekly commitment is 7 to 10 hours.
3. This file works as a scrum update.

---

### YYYY-MM-DD
**Total time:** 83 min

#### Hanon
- Piece: No. 5
- BPM: 70 + 140
- Techinique Focus: finger high action + evenness
- Time of Focus: 8 + 8min

#### Fundamentals
- [x] Scales
- [ ] Arpeggios
- [ ] Octaves
- [ ] Jumps
- [ ] Rotation
- [ ] Voicing
- [ ] Repeated notes
- [ ] Double notes (thirds and sixths)
- [ ] Hand synchronization
- [ ] Polyphonic control
- [ ] Chord techinique

- **Key:** Db major Bb Harmonic & melodic minor
- **BPM:** 140 1 notes per beat
- **Problem:** finger under techinique is not very proficient. It takes a second to cross thumb after it did its work
- **Time:** 15min

#### Etude
- Piece: Czerny Op849 No10
- BPM: 140 1 note per beat
- Problem: there are 2 sections not proficient. I marked it red. 
- Time of focus: ~30 min

#### Repertoire
- Piece: 巴赫旧约 arr by myself
- Section practiced: first page before the second phrase of the verse.
- BPM: 70-80-90-100. 1 quater note per beat
- Note: Great progress!
- Time of focus: 22 min

#### Today's win
> (one thing that was better than last time)
1. I changed 70 BPM 2 notes to 140 BPM 1 notes. The notes started fall exactly on the beat. I plan to stablise the beat techinique before moving on to 70BPM 2 notes.
2. I practiced Db major scale and its minor scales in good quality.
3. I covered the whole piece of the etude. everything is even and equally proficient. I also focused on musicality like crescendo, pianissimo, dynamic and piano. I spotted 2 worst sections and will cover it tommorow. I also completed the harmonic analysis and formal analysis. Spotted 2 harmonic mistakes written earlier.
4. I covered the first page at a speed of 100BPM 1 note per beat. everything was evenly proficient and the tempo is stable. 
5. I am quite satisfied with today's session.
6. I learnt that I might need to carry on playing Bach primary. I will leave it to future. I also learnt that after this, there are also little preludes to practice. 
7. I practiced finger high action and evenness. the tempo was quite steady.

#### Tomorrow's focus
>
1. carry on fundamental with a tweak: change 70BPM -> 140 BPM and higher if comfortable.
2. practice the not proficient part in etude and improve overall.
3. for scale, keep tempo steady.
4. maybe start do some arppegios from Db major. (explore methods to practice arppegios)
5. keep up with the consistency.
