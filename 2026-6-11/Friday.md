# 🎹 Piano Progress Log

## How to use
After every session, copy the template below and fill it in.

---

## Rules of Thumb
1. Practice with full attention; do not play on autopilot.
2. Prioritize control and musicality over raw BPM.
3. Practice slowly enough to stay accurate and relaxed.
4. Be honest about weak spots and work on the highest-leverage problem first.
5. Consistency matters more than intensity.
6. Stop or reset when focus is gone.

## Session Template
1. Each session belongs to a week folder.
2. Weekly commitment is 7 to 10 hours.
3. This file works as a scrum update.

---

### YYYY-MM-DD
**Total time:** 60 min

#### Fundamentals
- [x] Scales
- [ ] Arpeggios
- [ ] Octaves
- [ ] Jumps
- [ ] Rotation
- [ ] Voicing
- [ ] Repeated notes
- [ ] Double notes (thirds and sixths)
- [ ] Hand synchronization
- [ ] Polyphonic control
- [ ] Chord technique

- **Key:** Db major
- **BPM:** 70BPM 2 notes / beat
- **Problem:** still work on notes falling on beats exactly.
- **Time:** 15min

#### Etude
- Piece: Czerny Etude Op849 No10. first 8 bars
- BPM: 60BPM
- Problem: not proficient. accents
- Time of focus: 25min

#### Repertoire
- Piece: 巴赫旧约 arr. Liberte 
- Section practiced: first 8 bars
- BPM: 70-90 BPM 1 note / beat
- Note: accents
- Time of focus: 20min

#### Today's win
> (one thing that was better than last time)
1. ear is actually functioning. 
2. practiced slowly
3. record and listened to track my progress and if i am playing it right.
#### Tomorrow's focus
>
1. Bb minor scale
2. work on note falling on beats
3. fix accents on playing.
4. evenness in scale
