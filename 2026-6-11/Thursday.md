# 🎹 Piano Progress Log

## How to use
After every session, copy the template below and fill it in.

---

## Rules of Thumb
1. Practice with full attention; do not play on autopilot.
2. Prioritize control and musicality over raw BPM.
3. Practice slowly enough to stay accurate and relaxed.
4. Be honest about weak spots and work on the highest-leverage problem first.
5. Consistency matters more than intensity.
6. Stop or reset when focus is gone.

## Session Template
1. Each session belongs to a week folder.
2. Weekly commitment is 7 to 10 hours.
3. This file works as a scrum update.

---

### YYYY-MM-DD
**Total time:** 90 min

#### Fundamentals
- [x] Scales
- [ ] Arpeggios
- [ ] Octaves
- [ ] Jumps
- [ ] Rotation
- [ ] Voicing
- [ ] Repeated notes
- [ ] Double notes (thirds and sixths)
- [ ] Hand synchronization
- [ ] Polyphonic control
- [ ] Chord techinique

- **Key:**: Db Major
- **BPM:**: 70 BPM 2 notes / 140 BPM 1 notes
- **Problem:**: Fingers are not independent. Scales are not even. Notes do not always fall on beats
- **Time:**: ~50 min

#### Etude
- Piece: Czerny Etude Op.849 No.10
- BPM: 60 BPM 2 16 notes/beat
- Problem: Not proficient. 
- Time of focus: 15 min

#### Repertoire
- Piece: 巴赫旧约 arr. by myself
- Section practiced: Intro 
- BPM: 60 BPM 1 quater note per beat
- Note: Consistency is the key
- Time of focus: 15 min

#### Today's win
> (one thing that was better than last time)
1. Identifid vulnerabilities.
2. wrote piano logs to execute.
3. notes fall on beats when pay full attention.
#### Tomorrow's focus
>
1. Db Major scale + Bb Minor scales.
- goal: evenness and proficiency on both hands individually.
2. more time on etudes and repertoire.
